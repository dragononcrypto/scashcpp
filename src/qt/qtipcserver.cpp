...
#if defined MAC_OSX || defined __FreeBSD__ || defined LINUX
...
static bool ipcScanCmd(int argc, char *argv[], bool fRelay)
{
    ...
    {
        if (boost::algorithm::istarts_with(argv[i], "scash:"))
        {
...
void ipcInit(int argc, char *argv[])
{
    ...
    try {
        ...
        // Make sure we don't lose any scash: URIs
        for (int i = 0; i < 2; i++)
        {
